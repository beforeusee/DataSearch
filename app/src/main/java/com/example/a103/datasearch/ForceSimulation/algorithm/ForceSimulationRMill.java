...
public class ForceSimulationRMill {
    ...
    public ForceSimulationRMill(){

    }


    private static HashMap<String, Double> angleForceRMill(DataToolR toolR, DataCuttingPara cuttingPara, DataSimulationPara simulationPara, DataCutCoefficient coefficient, int i){
        ...
        //齿间距角度
        double angSpacing=2*PI/toolR.getToolTeethNum();
        //每齿进给
        double fz=cuttingPara.getFeedRate()/(cuttingPara.getSpindleSpeed()*toolR.getToolTeethNum());
        ...
        //切削线速度(m/min)
        double velocity=toolR.getRadius()*2*PI*cuttingPara.getSpindleSpeed()/1000;

        //时间(s)

        //刀齿循环
        for (int j=0;j<toolR.getToolTeethNum();j++){
            ...
            for (int k=0;k<numAxial;k++){
                ...
                //计算滞后因子
                double lagFactor=tan(toolR.getHelix()*PI/180)/toolR.getRadius();
                toolR.setLagFactor(lagFactor);

                //接触角沿轴向变化
                phi2=phi-zPos*toolR.getLagFactor();

                phi2=SimuBase.mod(phi2,2*PI);

                ////初始进刀时,切入切出角受圆角半径影响
                double zRadius=(zPos>=toolR.getRoundRadius())?toolR.getRadius():(toolR.getRadius()-toolR.getRoundRadius()+Math.sqrt(zPos*(toolR.getRoundRadius()*2-zPos)));
                double phi3=zPos>toolR.getRoundRadius()?PI/2:acos((toolR.getRoundRadius()-zPos)/toolR.getRoundRadius());
                ...
                double dBeta=simulationPara.getDz()*toolR.getLagFactor();
                double ds=sqrt(dzR*dzR+pow(zRadius*dBeta,2)+simulationPara.getDz()*simulationPara.getDz());
                ...
                //计算轴向半径的切削角
                //切入角
                if (toolR.getRadius()*cos(simulationPara.getPhiIn())>=zRadius){
                    phiIn=0;
                }else if (toolR.getRadius()*cos(simulationPara.getPhiIn())<=-zRadius){
                    phiIn=PI;
                }else {
                    phiIn=acos(toolR.getRadius()*cos(simulationPara.getPhiIn())/zRadius);
                }

                //切出角
                if (toolR.getRadius()*cos(simulationPara.getPhiOut())>=zRadius){
                    phiOut=0;
                }else if (toolR.getRadius()*cos(simulationPara.getPhiOut())<=-zRadius){
                    phiOut=PI;
                }else {
                    phiOut=acos(toolR.getRadius()*cos(simulationPara.getPhiOut())/zRadius);
                }
...
//                    chip.add(j,fz*sin(phi2)*sin(phi3));
                    if (h<0){
                        h=0;
                    }

                    //微元的力
                    if (zPos<toolR.getRoundRadius()){

                        dFt=coefficient.getKtc()*h*db+coefficient.getKte()*ds;
                        dFr=coefficient.getKrc()*h*db+coefficient.getKre()*ds;
                        dFa=-(coefficient.getKac()*h*db+coefficient.getKae()*ds);
                        ...
                        dFt=simulationPara.getDz()*(coefficient.getKtc()*h+coefficient.getKte());
                        dFr=simulationPara.getDz()*(coefficient.getKrc()*h+coefficient.getKre());
                        dFa=-simulationPara.getDz()*(coefficient.getKac()*h+coefficient.getKae());
        ...
        //计算扭矩(Nm)
        locT=locFt*toolR.getRadius()/1000;
        //计算功率(KW)
        ...
        pm+=locP;
        //弯矩(Nm)
        locBend=SimuBase.getToolBend(locFxy,toolR.getLenGauge()/1000);
     ...
     *       功率，扭矩，刀尖变形，夹持面弯矩
     * @param dataToolR 圆柱螺旋立铣刀
     * @param cuttingPara 切削参数
     * @param simulationPara 仿真参数
     * @param coefficient 切削力系数
     */
    public static void forceSimulationR(DataToolR dataToolR, DataCuttingPara cuttingPara, DataSimulationPara simulationPara, DataCutCoefficient coefficient){
        ...
        for (int i=0;i<(int)(2*PI*simulationPara.getPeriods()/simulationPara.getdPhi());i++){
            ...
            angleForce= angleForceRMill(dataToolR,cuttingPara,simulationPara,coefficient,i);
